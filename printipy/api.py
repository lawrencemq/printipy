    ...
    def __check_status(resp: Response, url: str):
        if resp.status_code == 400:
            ...
            raise InvalidScopeException(
                "This API key is not permitted to access this information."
            )
    ...
    def _parse(clazz, data: Union[List, Dict]):
        if isinstance(data, list):
            ...
            raise PrintiPyParseException(
                "Unable to parse response: was not a list or object"
            )
...
class PrintiPyCatalog(_ApiHandlingMixin):
    ...
    def get_print_provider(self, print_provider_id: Union[str, int]) -> PrintProvider:
        ...
        print_provider_url = (
            f"{self.api_url}/v1/catalog/print_providers/{print_provider_id}.json"
        )
...
class PrintiPyProducts(_ApiHandlingMixin, _ShopIdMixin):
    ...
    def get_products(
        self, shop_id: Union[str, int], max_pages: int = 1
    ) -> List[Product]:
        """
    ...
    @_ShopIdMixin._require_shop_id
    def create_product(
        self, create_product: CreateProduct, shop_id: Union[str, int]
    ) -> Product:
        """
        ...
        product_information = self._post(
            create_product_url, data=create_product.to_dict()
        )
        return self._parse(Product, product_information)
    ...
    def update_product(
        ...
        update_product_url = (
            f"{self.api_url}/v1/shops/{shop_id}/products/{product_id}.json"
        )
        product_information = self._put(
            update_product_url, data=update_product.to_dict()
        )
        return self._parse(Product, product_information)
    ...
    @_ShopIdMixin._require_shop_id
    def publish_product(
        self, product_id: str, publish: Publish, shop_id: Union[str, int]
    ) -> True:
        """
    ...
    def set_product_published_success(
        ...
        publishing_succeeded_url = (
            f"{self.api_url}/v1/shops/{shop_id}/products/"
...
class PrintiPyOrders(_ApiHandlingMixin, _ShopIdMixin):
    ...
    @_ShopIdMixin._require_shop_id
    def get_order(
        self, order_id: str, shop_id: Optional[Union[str, int]] = None
    ) -> Order:
        """
    ...
    @_ShopIdMixin._require_shop_id
    def send_order_to_production(
        self, order_id: str, shop_id: Union[str, int]
    ) -> Order:
        """
    ...
    def calc_shipping_for_order(
        ...
        shipping_estimate_url = (
            f"{self.api_url}/v1/shops/{shop_id}/orders/shipping.json"
        )
    ...
    def cancel_order(self, order_id: str, shop_id: Union[str, int]) -> Order:
        ...
        cancel_order_url = (
            f"{self.api_url}/v1/shops/{shop_id}/orders/{order_id}/cancel.json"
        )
...
class PrintiPyArtwork(_ApiHandlingMixin):
    ...
    def upload_artwork(
        self, filename: Optional[str] = None, url: Optional[str] = None
    ) -> Artwork:
        """
        ...
        if filename and url:
            raise PrintiPyException(
                "Must provide a local filename or url for upload, not both."
            )
        ...
        if url:
            ...
            raise PrintiPyException(
                "Must provide at least a local filename or url for upload."
            )
...
class PrintiPyWebhooks(_ApiHandlingMixin, _ShopIdMixin):
    ...
    @_ShopIdMixin._require_shop_id
    def create_webhook(
        self, create_webhook: CreateWebhook, shop_id: Union[str, int]
    ) -> Webhook:
        """
        ...
        webhook_information = self._post(
            create_webhook_url, data=create_webhook.to_dict()
        )
    ...
    def update_webhook(
        ...
        create_webhook_url = (
            f"{self.api_url}/v1/shops/{shop_id}/webhooks/{webhook_id}.json"
        )
        webhook_information = self._put(
            create_webhook_url, data=update_webhook.to_dict()
        )
        return self._parse(Webhook, webhook_information)
