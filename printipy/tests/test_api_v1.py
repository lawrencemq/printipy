...
class TestPrintiPyApiV1(TestCase):
    ...
    def prepare_response(
        self,
        http_type: Union[
            responses.GET, responses.POST, responses.DELETE, responses.PUT
        ],
        url: str,
...
class TestPrintiPyCatalogApiV1(TestPrintiPyApiV1):
    ...
    def test_get_blueprints(self):
        ...
        self.assertEqual(
            blueprints, [Blueprint.from_dict(x) for x in data_returned_from_url]
        )
        for blueprint in blueprints:
            for key in [
                ...
                self.assertIsNotNone(
                    blueprint.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_print_providers_for_blueprint(self):
        ...
        self.assertEqual(
            providers, [PrintProvider.from_dict(x) for x in data_returned_from_url]
        )
        for provider in providers:
            for key in ["id", "title"]:
                self.assertIsNotNone(
                    provider.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_variants(self):
        ...
        self.assertEqual(
            variant_info, PrintProviderVariants.from_dict(data_returned_from_url)
        )
        for variant in variant_info.variants:
            for key in ["id", "title", "options"]:
                self.assertIsNotNone(
                    variant.__getattribute__(key), f"{key} should not be None"
                )

            option = variant.options
            for key in ["color", "size"]:
                self.assertIsNotNone(
                    option.__getattribute__(key), f"{key} should not be None"
                )
            ...
            for placeholder in variant.placeholders:
                for key in ["position", "height", "width"]:
                    self.assertIsNotNone(
                        placeholder.__getattribute__(key), f"{key} should not be None"
                    )
    ...
    def test_get_shipping_info(self):
        ...
        self.assertEqual(
            shipping_info.handling_time.to_dict(), {"value": 30, "unit": "day"}
        )
        for profile in shipping_info.profiles:
            for key in ["variant_ids", "first_item", "additional_items", "countries"]:
                self.assertIsNotNone(
                    profile.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_print_providers(self):
        ...
        self.assertEqual(
            print_provider_info,
            [PrintProvider.from_dict(x) for x in data_returned_from_url],
        )
        ...
        for provider in print_provider_info:
            ...
            for key in ["address1", "city", "country", "region", "zip"]:
                self.assertIsNotNone(
                    provider.location.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_print_provider(self):
        ...
        self.assertEqual(
            print_provider_info, PrintProvider.from_dict(data_returned_from_url)
        )
...
class TestPrintiPyProductsApiV1(TestPrintiPyApiV1):
    ...
    def test_get_products(self):
        ...
        for product in product_info:
            for key in [
                ...
                self.assertIsNotNone(
                    product.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_product(self):
        ...
        self.assertEqual(
            self.api.products.get_product("121", shop_id=self.shop_id),
        ...
        for key in [
            ...
            self.assertIsNotNone(
                product_info.__getattribute__(key), f"{key} should not be None"
            )
    ...
    def test_create_product(self):
        ...
        product_info = self.api.products.create_product(
            CreateProduct.from_dict(data_for_url)
        )

        self.assertEqual(product_info, Product.from_dict(data_returned_from_url))
        self.assertEqual(
            self.api.products.create_product(
                CreateProduct.from_dict(data_for_url), shop_id=self.shop_id
            ),
            Product.from_dict(data_returned_from_url),
        )
    ...
    def test_create_product_raises_exception_without_shop_id(self):
        with self.assertRaises(PrintiPyException):
            ...
            api.products.create_product(
                CreateProduct.from_dict(self.data_for_create_product)
            )

    @responses.activate
    def test_update_product(self):
        data_for_url = {"title": "Product"}
        data_returned_from_url = {
            "id": "5d39b411749d0a000f30e0f4",
            "title": "Product",
            "description": "Good product",
        ...
        product_info = self.api.products.update_product(
            "121", UpdateProduct.from_dict(data_for_url)
        )

        self.assertEqual(product_info, Product.from_dict(data_returned_from_url))
        self.assertEqual(
            self.api.products.update_product(
                "121", UpdateProduct.from_dict(data_for_url), shop_id=self.shop_id
            ),
            Product.from_dict(data_returned_from_url),
        )
    ...
    def test_publish_product(self):
        ...
        self.assertTrue(
            self.api.products.publish_product("54321", Publish.from_dict(data_for_url))
        )
        self.assertTrue(self.api.products.publish_product("54321", Publish()))
        self.assertTrue(
            self.api.products.publish_product(
                "54321", Publish.from_dict(data_for_url), shop_id=self.shop_id
            )
        )
    ...
    def test_set_product_published_success(self):
        ...
        self.assertTrue(
            self.api.products.set_product_published_success(
                "54321", PublishingSucceeded.from_dict(data_for_url)
            )
        )
        self.assertTrue(
            self.api.products.set_product_published_success(
    ...
    def test_set_product_published_success_raises_exception_without_shop_id(self):
        ...
        with self.assertRaises(PrintiPyException):
            api = PrintiPy(api_token=self.test_api_token)
            api.products.set_product_published_success(
                "54321", PublishingSucceeded.from_dict(data_for_url)
            )
    ...
    def test_set_product_published_failed(self):
        ...
        self.assertTrue(
            self.api.products.set_product_published_failed("54321", "just because")
        )
        self.assertTrue(
            self.api.products.set_product_published_failed(
                "54321", "just because", shop_id=self.shop_id
            )
        )
    ...
    def test_unpublish_product(self):
        ...
        self.assertTrue(
            self.api.products.unpublish_product("54321", shop_id=self.shop_id)
        )
...
class TestPrintiPyOrdersApiV1(TestPrintiPyApiV1):
    ...
    def test_get_orders(self):
        ...
        self.assertEqual(
            self.api.orders.get_orders(max_pages=3, shop_id=self.shop_id), expected
        )
        ...
        for order in orders_info:
            for key in [
                ...
                self.assertIsNotNone(
                    order.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_get_order(self):
        ...
        self.assertEqual(
            self.api.orders.get_order("5a96f649b2439217d070f507", shop_id=self.shop_id),
            expected,
        )
        ...
        for key in [
            ...
            self.assertIsNotNone(
                order_info.__getattribute__(key), f"{key} should not be None"
            )
    ...
    def test_create_order_for_existing_product(self):
        ...
        data_returned_from_url = {"id": "5a96f649b2439217d070f507"}
        ...
        self.assertEqual(
            self.api.orders.create_order_for_existing_product(
                CreateOrderByExistingProduct.from_dict(data_for_url)
            ),
    ...
    def test_create_order_with_simple_image_positioning(self):
        ...
        self.assertEqual(
            self.api.orders.create_order_with_simple_image_positioning(
                CreateOrderBySimpleImageProcessing.from_dict(data_for_url)
            ),
            data_returned_from_url["id"],
        )
        self.assertEqual(
    ...
    def test_create_order_with_advanced_image_positioning(self):
        ...
        self.assertEqual(
            self.api.orders.create_order_with_advanced_image_positioning(
                CreateOrderByAdvancedImageProcessing.from_dict(data_for_url)
            ),
            data_returned_from_url["id"],
        )
        self.assertEqual(
    ...
    def test_create_order_with_print_details(self):
        ...
        self.assertEqual(
            self.api.orders.create_order_with_print_details(
                CreateOrderByPrintDetails.from_dict(data_for_url)
            ),
            data_returned_from_url["id"],
        )
        self.assertEqual(
            self.api.orders.create_order_with_print_details(
                CreateOrderByPrintDetails.from_dict(data_for_url), shop_id=self.shop_id
            ),
            data_returned_from_url["id"],
        )
    ...
    def test_create_order_with_sku(self):
        data_for_url = {
            ...
            "label": "00012",
            "line_items": [{"sku": "MY-SKU", "quantity": 1}],
            "shipping_method": 1,
            "send_shipping_notification": False,
            "address_to": {
                "first_name": "John",
        ...
        self.assertEqual(
            self.api.orders.create_order_with_sku(
                CreateOrderBySku.from_dict(data_for_url)
            ),
            data_returned_from_url["id"],
        )
        self.assertEqual(
            self.api.orders.create_order_with_sku(
                CreateOrderBySku.from_dict(data_for_url), shop_id=self.shop_id
            ),
            data_returned_from_url["id"],
        )
    ...
    def test_send_order_to_production(self):
        ...
        order_info = self.api.orders.send_order_to_production(
            "5d65c6ac01b403000a5d24d3"
        )

        self.assertEqual(order_info, Order.from_dict(data_returned_from_url))
        self.assertEqual(
            self.api.orders.send_order_to_production(
                "5d65c6ac01b403000a5d24d3", shop_id=self.shop_id
            ),
    ...
    def test_calc_shipping_for_order(self):
        ...
        data_returned_from_url = {"standard": 1000, "express": 5000}
        ...
        shipping_info = self.api.orders.calc_shipping_for_order(
        ...
        self.assertEqual(
            self.api.orders.calc_shipping_for_order(
                CreateShippingEstimate.from_dict(data_for_url), shop_id=self.shop_id
            ),
        ...
        for key in ["standard", "express"]:
            self.assertIsNotNone(
                shipping_info.__getattribute__(key), f"{key} should not be None"
            )

    def test_calc_shipping_for_order_raises_exception_without_shop_id(self):
        with self.assertRaises(PrintiPyException):
            api = PrintiPy(api_token=self.test_api_token)
            api.orders.calc_shipping_for_order(
                CreateShippingEstimate.from_dict(self.data_for_calc_shipping)
            )
    ...
    def test_cancel_order(self):
        ...
        self.assertEqual(
            self.api.orders.cancel_order(
                "5dee261dc400914833007902", shop_id=self.shop_id
            ),
            Order.from_dict(data_returned_from_url),
        )
...
class TestPrintiPyArtworkApiV1(TestPrintiPyApiV1):
    ...
    def test_get_artwork_uploads(self):
        ...
        for artwork in artwork_info:
            for key in [
                ...
                self.assertIsNotNone(
                    artwork.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_upload_artwork_with_filename(self):
        ...
        for key in [
            ...
            self.assertIsNotNone(
                artwork_info.__getattribute__(key), f"{key} should not be None"
            )
    ...
    def test_upload_artwork_with_url(self):
        ...
        artwork_info = self.api.artwork.upload_artwork(
            url="http://somekindofimage.com/image.png"
        )
...
class TestPrintiPyWebhooksApiV1(TestPrintiPyApiV1):
    ...
    def test_get_shop_webhooks(self):
        ...
        for webhook in webhooks_info:
            for key in ["topic", "url", "shop_id", "id"]:
                self.assertIsNotNone(
                    webhook.__getattribute__(key), f"{key} should not be None"
                )
    ...
    def test_create_webhook(self):
        ...
        self.assertEqual(
            self.api.webhooks.create_webhook(CreateWebhook.from_dict(data_for_url)),
            Webhook.from_dict(data_returned_from_url),
        )
        self.assertEqual(
            self.api.webhooks.create_webhook(
                CreateWebhook.from_dict(data_for_url), shop_id=self.shop_id
            ),
            Webhook.from_dict(data_returned_from_url),
        )
    ...
    def test_update_webhook(self):
        data_for_url = {"url": "https://example.com/callback/order/created"}
        data_returned_from_url = {
            "topic": "order:created",
        ...
        self.assertEqual(
            self.api.webhooks.update_webhook(
                "5cb87a8cd490a2ccb256cec4", UpdateWebhook.from_dict(data_for_url)
            ),
            Webhook.from_dict(data_returned_from_url),
        )
        self.assertEqual(
            self.api.webhooks.update_webhook(
                "5cb87a8cd490a2ccb256cec4",
